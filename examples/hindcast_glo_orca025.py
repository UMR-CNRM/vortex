...
for i in range(1, 16):
    id_moor = Term(i)
    resources_list.append( toolbox.rload(
        rd, kind='moorings', grid=grid, term=id_moor,
...
for id_block in range(1, 5):
    resources_list.append( toolbox.rload(
        rd, kind = 'binary', assim=True, type='block', term=id_block,
        local=local_dir+'main_block_'+str(id_block)).pop()
    )

resources_list.append( toolbox.rload(
    rd, kind = 'binary', assim=True, type='pil', term=0,
    local=local_dir+'SAMIAU_PALM_MULTIMP.pil').pop()
)

resources_list.append( toolbox.rload(
    rd, kind = 'binary', assim=False, term=0, type='build_nc',
    local=local_dir+'build_nc').pop()
)

resources_list.append( toolbox.rload(
    rd, kind = 'binary', assim=True, term=0, type='anolist',
...
for f in clim_fields:
    resources_list.append( toolbox.rload(
        rd, kind = 'climatology', grid=grid, year=clim_year, month=clim_month, field=f,
...
resources_list.append( toolbox.rload(
    rd, kind='namelist', assim=True, nmtype='kernel',
    local=local_dir+'kernel.prm').pop()
)

resources_list.append( toolbox.rload(
    rd, kind='namelist', assim=True, nmtype='palm',
...
for bogus in bogus_namelist:
    resources_list.append( toolbox.rload(
        rd, kind='namelist', assim=True, nmtype=bogus[0],
