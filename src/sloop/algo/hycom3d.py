...
# from sloop.cf import get_x_dim, get_y_dim
# from sloop.times import get_best_estimate
...
class Hycom3dModelCompilator(Hycom3dCompilator):
    ...
    def prepare(self, rh, kw):
        ...
        # Check dimensions
        hycrgs = self.context.sequence.effective_inputs(role="Hycom3dRegionalGrid")
        if len(hycrgs) == 0:
            raise AlgoComponentError("No Hycom3d regional grid found")
        regional_grid_basename = hycrgs[0].rh.container.localpath()[:-4]
