...
class ZSVTool(footprints.FootprintBase):
    ...
    _collector = ('zsvtool',)
    _footprint = dict(
        info = 'Abstract ZSV Miscellaneous Tool',
        attr = dict(
            kind = dict(),
            name = dict(
                optional = True,
...
class ZSVDriver(ZSVTool):
    ...
    _footprint = dict(
        ...
        attr = dict(
            ...
            fileconf = dict(
                optional = True,
                default  = DelayedEnvValue('ZSV_CONF_PATH', 'zsv-driver-conf.json'),
            ),
            rootpath = dict(
                optional = True,
                default  = DelayedEnvValue('ZSV_ROOT_PATH', vortex.sh().pwd()),
            ),
            workdir = dict(
                optional = True,
                default  = DelayedEnvValue('ZSV_WORK_PATH', vortex.sh().pwd()),
            ),
            bkup_path = dict(
                optional = True,
                default  = DelayedEnvValue('ZSV_BKUP_PATH', vortex.sh().abspath('../bkup')),
            ),
            sites = dict(
                optional = True,
    ...
    def __init__(self, *args, **kw):
        ...
        # swith log level to actual choice
    ...
    def setup_config(self):
        ...
        logger.info('Configuration <file:%s>', self.fileconf)
        self.config = self.sh.json_load(self.fileconf)
...
class ZSVQualityStats(ZSVDriver):
    ...
    def record_load(self):
        ...
        if self.update:
            self.upfile = self.sh.path.join(self.workdir, 'zsv-record-' + self.name + '.json')
