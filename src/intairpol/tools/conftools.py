...
class MocageDomainsConfTool(ConfTool):
    ...
    def _add_domain_def(self, dname, ddef):
        ...
        ddef.setdefault('post_outgrib2', '')
        ddef.setdefault('restart_steps', '')
        ddef.setdefault('stats_steps', '')
        ddef.setdefault('atm_cpl_fcst_steps', '')
        # Generic transform
    ...
    def _domain_any_steps(self, cutoff, hh, entry, start, end,
        ...
        # case of DEV run with endFC less than 96 and check_inputs try to resolve more Time steps than existing ones
        if start > end:
            start = end
    ...
    @_add_start_end_doc
    @_add_cutoff_hh_doc
    def atm_cpl_fcst_steps(self, cutoff, hh, start=0, end=None, final=True, first=True):
        """The geometry/atm_cpl_fcst_steps :mod:`footprints`' substitution dictionary."""
        return self._domain_any_steps(cutoff, hh, 'atm_cpl_fcst_steps', start, end,
                                      final=final, first=first)
    ...
    @_add_start_end_doc
    @_add_cutoff_hh_doc
    def atm_cpl_shiftedsteps_fcst(self, cutoff, hh, start=0, end=None, final=True, first=True):
        """The geometry/shifted atm_cpl_fcst_steps :mod:`footprints`' substitution dictionary."""
        return self._domain_any_steps(cutoff, hh, 'atm_cpl_fcst_steps', start, end,
                                      shift=True, final=final, first=first)
    ...
    @_add_start_end_doc
    @_add_cutoff_hh_doc
    def post_outgrib2(self, cutoff, hh, start=0, end=None, final=True, first=True):
        """The list of time for post_outgrib2 :mod:`footprints`' substitution dictionary."""
        return self._domain_any_steps(cutoff, hh, 'post_outgrib2', start, end,
                                      final=final, first=first)
...
class MocageMixedDomainsInfo(object):
    ...
    def _domain_any_steps(self, entry, start, end, shift=False, final=True, first=True):
        ...
        if start > end:
            start = end
            logger.info('**** domain_any_steps : ajust start to end value ', start)
    ...
    @_add_start_end_doc
    def atm_cpl_shiftedsteps_fcst(self, start=0, end=None, final=True, first=True):
        """The sifted surf_cpl_steps of coupling data."""
        return self._domain_any_steps('atm_cpl_fcst_steps', start, end,
                                      shift=True, final=final, first=first)
